# Single Tunnel Control API Documentation

## Overview

This document describes the newly added single tunnel control functionality in the autossh-tunnel-dockerized project, which allows you to independently start, stop, and manage individual SSH tunnels without affecting other running tunnels.
...
Tunnel state is saved in `/tmp/autossh_tunnels.state` file, containing:
...
Tunnels marked as `interactive: true` require manual password input and won't start automatically. These tunnels show as `INTERACTIVE` status in the list.
...
- [SSH Configuration Guide](README_ssh_config_en.md)
- [Chinese Documentation](tunnel-control-api_zh.md)

## Changelog

- **2026-01-25**: Added single tunnel control API
  - Added `start-tunnel` and `stop-tunnel` commands
  - Implemented HTTP API endpoints `/start/<hash>` and `/stop/<hash>`
  - Improved state management and logging system

...
![隧道详情页（浅色主题）](assets/images/tunnel-detail.png)

![隧道详情页（深色主题）](assets/images/tunnel-detail-dark.png)
...
!!! tip "WebSocket 配置要求"
    ...
    详见下方 [WebSocket 配置](#websocket-配置) 章节。
...
3. **WebSocket 终端**（可选）：浏览器通过 WebSocket 连接到 ws-server（默认端口 8022）进行交互式认证
...
- **浏览器内认证**：通过 WebSocket 在浏览器中直接完成交互式认证
